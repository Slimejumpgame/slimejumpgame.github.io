...
function assertRegistryCountsAndExistingIds() {
  ...
  const requirements = Array.from(
    {length: unlockableWardrobeItems},
    (_, index) => 15 + index * 5
  );
  assert.equal(requirements.length, unlockableWardrobeItems);
  assert.equal(requirements.at(-1), 355);
  assert.match(wardrobeSource, /const WARDROBE_UNLOCK_START = 15;/);
  assert.match(wardrobeSource, /const WARDROBE_UNLOCK_STEP = 5;/);
  assert.match(
    wardrobeSource,
    /\(SLIME_COLOR_ORDER\.length - 1\) \+[\s\S]*?UNLOCKABLE_SLIME_COSMETICS\.length \+[\s\S]*?UNLOCKABLE_SLIME_BEARDS\.length/
  );
